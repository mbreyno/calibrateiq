...
SELECT qr.id, c.first_name, c.last_name, c.date_of_birth,
       ...
       qr.q1 AS stored_q1,
       CASE
         WHEN EXTRACT(YEAR FROM AGE(NOW(), c.date_of_birth))::int < 45  THEN 50
         WHEN EXTRACT(YEAR FROM AGE(NOW(), c.date_of_birth))::int <= 55 THEN 40
         WHEN EXTRACT(YEAR FROM AGE(NOW(), c.date_of_birth))::int <= 65 THEN 30
         WHEN EXTRACT(YEAR FROM AGE(NOW(), c.date_of_birth))::int <= 75 THEN 20
         ELSE 10
       END AS expected_q1
